					...
					<!-- Panel Static Labels -->
		          	<div class="panel">
			            ...
			            <div class="panel-body container-fluid">
			              	<form action="<?=base_url('Admin/create_expenses');?>" id="productAdd" name="productAdd" class="form-horizontal" method="post" enctype="multipart/form-data">
                              <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" 
                                    value="<?=$this->security->get_csrf_hash();?>" >
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="position">พื้นที่</label>
				                  	<input type="text" class="form-control" id="position" name="position" placeholder="เช่น ต่างจังหวัดในรัศมี 101 km - 300 km จากอนุเสาวรีย์ (จะคิดกิโลเมตรละ 12 บาท)" required>
				                </div>
                                
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="distance">ระยะทางไม่เกิน(รัศมี) กม. **ระบุตัวเลขเท่านั้น</label>
				                  	<input type="text" class="form-control" id="distance" name="distance" placeholder="300" onkeypress='return event.charCode >= 48 && event.charCode <= 57' autocomplete="off" required>
				                </div>
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="tr_price">ราคา/กม.</label>
				                  	<input type="text" class="form-control" id="tr_price" name="tr_price" placeholder="เช่น 12" onkeypress='return event.charCode >= 48 && event.charCode <= 57' autocomplete="off" required>
				                </div>
