...
<body class="">
			                  	...
			                  	<div class="bootstrap-table">
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?=base_url('Admin/form_gallery');?>';">
			                  		...
			                  		<div class="" style="padding-bottom: 0px;">
										  ...
										  <ul>
													...
													<!-- <ol>
														สถานะ NEW Gallery ปรับได้ 4 รายการ
													</ol> -->
													<ol>
			                  				...
			                  				<table id="exampleTableToolbar" data-mobile-responsive="true" class="table table-hover" data-pagination="true" data-search="true" style="margin-top: 0px;">
			                    				...
			                    				<?php if(!empty($galleries)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($galleries as $val_item) : ?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td> 
			                  							<td style=""><?=$val_item['name']; ?></td>
														<td style="">
															<a type="button" href="<?=base_url('Admin/gallery_albums/'.$val_item['id'].'/?slug='.$val_item['name']);?>" class="btn btn-outline-primary" data-toggle="tooltip" data-placement="top" title="File upload">
														...
														<td style="">
															<?php if($isrecom == 8) : ?>
																<?php if ($val_item['is_recommend'] == '1') : ?>
			                  										<button class="btn btn-sm btn-success btn-icon btn-floating" type="button" onclick="gChange(<?=$val_item['id']; ?>, 0)"><i class="icon wb-check" aria-hidden="true"></i></button>
			                  									<?php else : ?>
															...
															<?php else : ?>
																<?php if ($val_item['is_recommend'] == '1') : ?>
			                  										<button class="btn btn-sm btn-success btn-icon btn-floating" type="button" onclick="gChange(<?=$val_item['id']; ?>, 0)"><i class="icon wb-check" aria-hidden="true"></i></button>
			                  									<?php else : ?>
			                  										<button class="btn btn-sm btn-danger btn-icon btn-floating" type="button" onclick="gChange(<?=$val_item['id']; ?>, 1)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  									<?php endif; ?>
															<?php endif; ?>
														...
														<td style="">
			                  							...
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/edit_gallery/'.$val_item['id']);?>';" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icon wb-pencil" aria-hidden="true"></i></button>
		...
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<h6>สามารถเปลี่ยนสถานะ Active/Deactive ได้ทุกเมื่อ</h6>
				</div>
	...
	<script type="text/javascript">
		...
		function changeIsRecommend() {
            $.ajax({
				type 	: 'POST',
				url 	: '<?=base_url("Admin/change_GalleryIsRecommend"); ?>',
				data 	: {id:$('#g_id').val(),st:$('#p_st').val(), action:'change', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
				success: function(data) {
					if (data = 'true') {
						alert("Success, to OK");
						location.reload();
