...
<body class="">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<!-- <div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?//=base_url('Admin/form_about');?>';">
			                  		...
			                  		<div class="" style="padding-bottom: 0px;">
										  ...
										  <ul>
			                  				...
			                  				<table id="exampleTableToolbar" data-mobile-responsive="true" class="table table-hover" data-pagination="true" data-search="true" style="margin-top: 0px;">
			                    				...
			                    				<?php if(!empty($aboutusies)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($aboutusies as $val_item) : 
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td> 
														...
														<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/edit_about/'.$val_item['id']);?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delAbout(<?=$val_item['id'];?>,<?=$val_item['is_active'];?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<input type="hidden" id="ab_st" name="ab_st">
	...
	<script type="text/javascript">
		...
		function deleteAbout()
		{
			$.ajax({
				type 	: 'POST',
				url 	: '<?=base_url("Admin/delAbout"); ?>',
				data 	: {id:$('#ab_id').val(),st:$('#ab_st').val(), action:'delAbout', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
