...
<head>
	...
	<title>Edit Portfolio | Big Inspector </title>
...
<body class="">
	...
	<?php 
		...
		// multiple image
		$FileUploader=$this->Upload_portfolio_model->fileUploaded($port_descs[0]['id']);
					...
					<!-- Panel Static Labels -->
		          	<div class="panel">
			            ...
			            <div class="panel-body container-fluid">
			              	<form action="<?=base_url('Admin/update_portfolio');?>" id="productEdit" name="productEdit" class="form-horizontal" method="post" enctype="multipart/form-data">
                              <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" 
                                    value="<?=$this->security->get_csrf_hash();?>" >
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="title">Name</label>
				                  	<input type="text" class="form-control" id="name" name="name" value="<?=$port_descs[0]['name'];?>" placeholder="Name" required>
				                </div>
                                <div class="form-group form-material" data-plugin="formMaterial">
				                  	<label class="form-control-label" for="title">คำอธิบาย</label>
				                  	<input type="text" class="form-control" id="caption" name="caption" value="<?=$port_descs[0]['short_dsc'];?>" placeholder="Name" required>
				                </div>
                                <div id="category" class="form-group form-material col-md-6" data-plugin="formMaterial">
                                    ...
                                    <select class="form-control" name="category">
                                        <?php foreach($categories as $cate_dsc) : ?>
                                            <option value="<?=$cate_dsc['id']; ?>" <?php if($port_descs[0]['type_id'] == $cate_dsc['id']){ echo 'selected'; } ?>><?=$cate_dsc['name']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
								...
								<div class="form-group form-material form-material-file" data-plugin="formMaterial">
			                      	...
			                      	<input type="file" id="covImg" name="covImg" data-plugin="dropify" data-default-file="<?=base_url('./assets/images/portfolio/cover/'.$port_descs[0]['id'].'/'.$port_descs[0]['img_cover']);?>" data-allowed-file-extensions="png jpg jpeg PNG JPG JPEG"/>
								...
								<div class="form-group form-material form-material-file" data-plugin="formMaterial">
                                  ...
                                  <input type="hidden" name="port_id" value="<?=$port_descs[0]['id']; ?>">
